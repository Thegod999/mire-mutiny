...
public class Enemy : MonoBehaviour
{
    ...
    public bool goFindPath = false;
    void Start()
    {
      float posX = -map.width/-2 + Mathf.Round(transform.position.x) - 0.5f;
      float posY = -map.height/-2 + Mathf.Round(transform.position.y) - 0.5f;
      startSpot = new Vector2(posX, posY);


      if (Stray == true) {
        moveSpot = new Vector2(Random.Range(transform.position.x - MoveX,transform.position.x + MoveX), Random.Range(transform.position.y - MoveY, transform.position.y + MoveY));
        goFindPath = true;
      }
    }
  ...
  //  {
    //  if (goFindPath == true) {
    //    FindPath(startSpot, moveSpot);//Find a path to the goal
    //  }
  //  }
